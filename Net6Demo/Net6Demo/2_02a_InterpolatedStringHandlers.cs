...
namespace Net6Demo
{
    internal static class OwnInterpolatedStringHandlers
    {
        public static StringBuilder AppendWithHandler(this StringBuilder stringBuilder, IFormatProvider? provider,
            [InterpolatedStringHandlerArgument("stringBuilder", "provider")]
            ref MyAppendInterpolatedStringHandler handler) => stringBuilder;
        


        public static void Test()
        {
            var (name, age, list) = ("Mike", 15, new List<string> { "A", "B", "C" });

            var s1 = string.Create(new MyFormatter(), stackalloc char[64], $"{nameof(name)}: {name}, {nameof(age)}: {age}, list: {list}");

            
            var sb = AppendWithHandler(new("SB: "), new MyFormatter(), $"{nameof(name)}: {name}, {nameof(age)}: {age}, list: {list}");
            var s2 = sb.ToString();
        }
        ...
        class MyFormatter : IFormatProvider, ICustomFormatter
        {
            ...
            public string Format(string? format, object? arg, IFormatProvider? provider) =>
                arg switch
                {
                    IEnumerable enumerable => Nemesis.TextParsers.TextTransformer.Default.GetTransformer(enumerable.GetType()).FormatObject(enumerable),
                    _ => (arg as IFormattable)?.ToString(format, CultureInfo.InvariantCulture) ?? arg?.ToString() ?? ""
                };
        }

        [EditorBrowsable(EditorBrowsableState.Never)]
        [InterpolatedStringHandler]
        public struct MyAppendInterpolatedStringHandler
        {
            // Implementation note:
            ...
            public MyAppendInterpolatedStringHandler(int literalLength, int formattedCount, StringBuilder stringBuilder)
            {
            ...
            /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
            public MyAppendInterpolatedStringHandler(int literalLength, int formattedCount, StringBuilder stringBuilder, IFormatProvider? provider)
            {
                _stringBuilder = stringBuilder;
                _provider = provider;
                _hasCustomFormatter = provider is not null && MyDefaultInterpolatedStringHandler.HasCustomFormatter(provider);
            }
            ...
            private void AppendFormattedWithTempSpace<T>(T value, int alignment, string? format)
            {
                ...
                var handler = new MyDefaultInterpolatedStringHandler(0, 0, _provider, stackalloc char[256]);
        ...
        [InterpolatedStringHandler]
        public ref struct MyDefaultInterpolatedStringHandler
        {
            // Implementation note:
            ...
            /// <param name="formattedCount">The number of interpolation expressions in the interpolated string.</param>
            /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
            public MyDefaultInterpolatedStringHandler(int literalLength, int formattedCount)
            {
                _provider = null;
            ...
            /// <param name="provider">An object that supplies culture-specific formatting information.</param>
            /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
            public MyDefaultInterpolatedStringHandler(int literalLength, int formattedCount, IFormatProvider? provider)
            {
                _provider = provider;
                _chars = _arrayToReturnToPool = ArrayPool<char>.Shared.Rent(GetDefaultLength(literalLength, formattedCount));
                _pos = 0;
            ...
            /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
            public MyDefaultInterpolatedStringHandler(int literalLength, int formattedCount, IFormatProvider? provider, Span<char> initialBuffer)
            {
                _provider = provider;
                ...
                /*if (value.Length == 2)
                {
                    ...
                    if ((uint)pos < chars.Length - 1)
                    {
                        System.Runtime.CompilerServices.Unsafe.WriteUnaligned(
                            ref System.Runtime.CompilerServices.Unsafe.As<char, byte>(ref System.Runtime.CompilerServices.Unsafe.Add(ref MemoryMarshal.GetReference(chars), pos)),
                            System.Runtime.CompilerServices.Unsafe.ReadUnaligned<int>(ref System.Runtime.CompilerServices.Unsafe.As<char, byte>(ref GetRawStringData(value)   )));
                        _pos = pos + 2;
                    }
                    else
                    {
            ...
            private void GrowCore(uint requiredMinCapacity)
            {
                ...
                uint newCapacity = Math.Max(requiredMinCapacity, Math.Min((uint)_chars.Length * 2, 20_000));
